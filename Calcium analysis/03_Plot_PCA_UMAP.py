...
def plot_dimensionality_reduction(
    ...
    # Add feature loadings or correlations if applicable
...
def create_analysis_report(output_folder, cell_data, sync_matrix):
    ...
    fig = plot_dimensionality_reduction(
        ...
        title='PCA of Cell Features by Marker Expression',
        dual_color_markers=True
    )
    if fig is not None:
        output_path = os.path.join(output_folder, "07_marker_pca.tiff")
    ...
    fig = plot_dimensionality_reduction(
        ...
        title='PCA of Cell Features by dependancy Expression',
        dual_color_markers=True
    )
    if fig is not None:
        output_path = os.path.join(output_folder, "07b_dependancy_pca.tiff")
        fig.savefig(output_path, format='tiff', dpi=300)
        plt.close(fig)
        saved_files.append(output_path)
    ...
    fig = plot_dimensionality_reduction(
        ...
        label_by='marker', 
        marker_group='Time',
        title='PCA of Cell Features by Time Expression',
        dual_color_markers=False
    )
    if fig is not None:
        output_path = os.path.join(output_folder, "07c_Time_pca.tiff")
        fig.savefig(output_path, format='tiff', dpi=300)
        plt.close(fig)
        saved_files.append(output_path)
    ...
    fig = plot_dimensionality_reduction(
        ...
        data_source='tracks', 
        label_by='community',
        title='PCA of Cell Activity Tracks'
    )
    if fig is not None:
        output_path = os.path.join(output_folder, "08_track_pca.tiff")
        fig.savefig(output_path, format='tiff', dpi=300)
        plt.close(fig)
        saved_files.append(output_path)
    ...
    if fig is not None:
        output_path = os.path.join(output_folder, "09b_Time_umap.tiff")
        fig.savefig(output_path, format='tiff', dpi=300)
        plt.close(fig)
        saved_files.append(output_path)

    # Plot 10: Track UMAP by community
    fig = plot_dimensionality_reduction(
        cell_data, 
        method='umap', 
        data_source='tracks', 
        label_by='community',
        title='UMAP of Cell Activity Tracks'
    )
    if fig is not None:
    ...
    # Plot 10b: Track UMAP by marker
    fig = plot_dimensionality_reduction(
        cell_data, method='umap', data_source='tracks', 
        label_by='marker', marker_group='Lineage',
        title='UMAP of Cell Activity Tracks by Contribution',
        dual_color_markers=True)
    
    if fig is not None:
        output_path = os.path.join(output_folder, "10b_track_umap_by_Contribution.tiff")
        fig.savefig(output_path, format='tiff', dpi=300)
        plt.close(fig)
        saved_files.append(output_path)
    ...
    fig = plot_dimensionality_reduction(
        ...
        label_by='marker',
        marker_group='Dependancy',
        title='UMAP of Cell Activity Tracks by Dependancy',
        dual_color_markers=True
    )
    if fig is not None:
    ...
    fig = plot_dimensionality_reduction(
        ...
        label_by='marker',
        marker_group='Time',
        title='UMAP of Cell Activity Tracks by Time',
        dual_color_markers=False
    )
    if fig is not None:
